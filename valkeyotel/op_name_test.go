...
func TestDefaultOpNameResolver_OpName(t *testing.T) {
	resolver := valkeyotel.DefaultOpNameResolver{}
...
func TestDefaultOpNameResolver_MultiOpName(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			resolver := valkeyotel.DefaultOpNameResolver{Limit: tt.limit}
...
func TestDefaultOpNameResolver_MultiCacheableOpName(t *testing.T) {
	...
	tests := [...]struct {
		...
		},
		{
			name: "single command",
			cmds: []valkey.CacheableTTL{
				valkey.CT(builder.Get().Key("one").Cache(), time.Minute),
			},
			want: "GET",
		},
		{
			...
			cmds: []valkey.CacheableTTL{
				valkey.CT(builder.Get().Key("one").Cache(), time.Minute),
				valkey.CT(builder.Mget().Key("two").Cache(), time.Minute),
				valkey.CT(builder.Strlen().Key("three").Cache(), time.Minute),
			},
			want: "GET MGET STRLEN",
		},
		{
