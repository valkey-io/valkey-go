...
func setup(t testing.TB) valkey.Client {
...
func TestRateLimiter(t *testing.T) {
	...
	t.Run("Check allowed with limit option", func(t *testing.T) {
		...
		result, err = limiter.Check(context.Background(), key, valkeylimiter.WithCustomRateLimit(10, time.Millisecond*100))
		if err != nil {
			t.Fatal(err)
		}
...
func generateLoad(t *testing.T, limiter valkeylimiter.RateLimiterClient, key string, n int) {
	for i := 0; i < n; i++ {
