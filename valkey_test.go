...
func TestNewClusterClientError(t *testing.T) {
	...
	t.Run("cluster slots command error", func(t *testing.T) {
		...
		go func() {
			...
			mock.Expect("CLIENT", "SETINFO", "LIB-VER", LibVer).
				ReplyError("UNKNOWN COMMAND")
			mock.Expect("CLUSTER", "SLOTS").Reply(ValkeyMessage{typ: '-', string: "other error"})
...
func TestFallBackSingleClient(t *testing.T) {
	...
	go func() {
		...
		mock.Expect("CLIENT", "SETINFO", "LIB-VER", LibVer).
			ReplyError("UNKNOWN COMMAND")
		mock.Expect("CLUSTER", "SLOTS").Reply(ValkeyMessage{typ: '-', string: "ERR This instance has cluster support disabled"})
		mock.Expect("PING").ReplyString("OK")
		mock.Close()
		close(done)
	}()
