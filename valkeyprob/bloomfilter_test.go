...
func setupValkey7Cluster() (valkey.Client, func() error, error) {
	client, err := valkey.NewClient(
		valkey.ClientOption{InitAddress: []string{"127.0.0.1:7001", "127.0.0.1:7002", "127.0.0.1:7003"}},
	)
	if err != nil {
...
func setupValkey5Cluster() (valkey.Client, func() error, error) {
	client, err := valkey.NewClient(
		valkey.ClientOption{
...
func TestNewBloomFilter(t *testing.T) {
	t.Run("default", func(t *testing.T) {
		client, flushAllAndClose, err := setupValkey7Cluster()
		if err != nil {
			t.Error(err)
		}
		defer func() {
	...
	t.Run("enable read operation when supported valkey version", func(t *testing.T) {
		client, flushAllAndClose, err := setupValkey7Cluster()
		if err != nil {
			t.Error(err)
		}
...
func TestBloomFilterExistsError(t *testing.T) {
	...
	t.Run("unsupported valkey version for read operation", func(t *testing.T) {
		client, flushAllAndClose, err := setupValkey5Cluster()
		if err != nil {
			t.Error(err)
		}
		defer func() {
		...
		if !strings.Contains(err.Error(), "unknown command") {
			t.Error("Error is not unsupported valkey version")
		}
	})
}
...
func TestBloomFilterDelete(t *testing.T) {
	t.Run("delete exists", func(t *testing.T) {
		...
		if !valkey.IsValkeyNil(resp.Error()) {
			t.Error("Error is not valkey.ErrNil")
		}
