...
func makeClient(t *testing.T, addr []string) CacheAsideClient {
	client, err := NewClient(ClientOption{
		ClientOption: valkey.ClientOption{InitAddress: addr, PipelineMultiplex: -1, SelectDB: 5},
...
func TestWithClientBuilder(t *testing.T) {
	...
	c, err := NewClient(ClientOption{
		ClientOption: valkey.ClientOption{InitAddress: addr, PipelineMultiplex: 3, DisableAutoPipelining: true, SelectDB: 5},
...
func BenchmarkGet(b *testing.B) {
	...
	// Populate the key and warm the valkey client-side cache so subsequent
	// Get calls hit the cache and exercise the valkeyaside fast path only.
...
func (c *cancelAfterAcquireClient) Do(ctx context.Context, cmd valkey.Completed) valkey.ValkeyResult {
	if isAcquireCommand(cmd.Commands(), c.key) {
		resp := c.Client.Do(ctx, cmd)
		if valkey.IsValkeyNil(resp.Error()) && c.accepted.CompareAndSwap(false, true) {
			c.cancel()
			return valkey.NewErrorResult(ctx.Err())
		}
		return resp
	}
...
func (c *blockingAcquireClient) Do(ctx context.Context, cmd valkey.Completed) valkey.ValkeyResult {
	if isAcquireCommand(cmd.Commands(), c.key) {
		if c.calls.Add(1) == 1 {
			close(c.started)
		}
...
func (c *blockingKeepaliveClient) Do(ctx context.Context, cmd valkey.Completed) valkey.ValkeyResult {
...
func TestAcquireCancellationCleansPlaceholder(t *testing.T) {
	for _, useLuaLock := range []bool{false, true} {
		...
		t.Run(name, func(t *testing.T) {
			...
			client, err := NewClient(ClientOption{
				ClientOption: valkey.ClientOption{InitAddress: addr, DisableAutoPipelining: true, SelectDB: 5},
				ClientTTL:    time.Second,
			...
			if !wrapped.accepted.Load() {
				t.Fatal("Valkey did not accept the lock command")
			}
			if loaderCalled {
...
func TestSingleflightStartsBeforeKeepalive(t *testing.T) {
	...
	client, err := NewClient(ClientOption{
		ClientOption: valkey.ClientOption{InitAddress: addr, SelectDB: 5},
