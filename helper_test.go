		...
		t.Run("Delegate DisabledCache DoCache", func(t *testing.T) {
			...
			m.DoMultiFn = func(cmd ...Completed) *valkeyresults {
				...
				for i, c := range cmd {
					...
					values := make([]ValkeyMessage, len(commands)-1)
	...
	t.Run("single client", func(t *testing.T) {
		...
		t.Run("Delegate Do", func(t *testing.T) {
			m.DoFn = func(cmd Completed) ValkeyResult {
				...
				return NewResult(ValkeyMessage{typ: ':', intlen: 1}, nil)
			}
		...
		t.Run("Delegate Do", func(t *testing.T) {
			...
			m.DoMultiFn = func(cmd ...Completed) *valkeyresults {
				...
				for i, c := range cmd {
					...
					values := make([]ValkeyMessage, len(commands)-2)
...
func TestDecodeSliceOfJSON(t *testing.T) {
	...
	result := ValkeyResult{val: slicemsg('*', values)}
	...
	t.Run("Scan []*T: has nil error message", func(t *testing.T) {
		...
		hasNilResult := ValkeyResult{val: slicemsg('*', hasNilValues)}
	...
	t.Run("has non-nil error message in result", func(t *testing.T) {
		...
		hasErrResult := ValkeyResult{val: slicemsg('*', hasErrValues)}
...
func TestClusterHelpersMgetcmdspRecycle(t *testing.T) {
	...
	t.Run("doMultiSet_skips_mgetcmdsp_Put_when_any_non_valkey_error", func(t *testing.T) {
		m, client := cluster()
		m.DoMultiFn = func(cmd ...Completed) *valkeyresults {
	...
	t.Run("doMultiSet_Put_mgetcmdsp_when_only_valkey_level_errors", func(t *testing.T) {
		m, client := cluster()
		m.DoMultiFn = func(cmd ...Completed) *valkeyresults {
			out := make([]ValkeyResult, len(cmd))
			for i := range cmd {
		...
		for k, e := range errs {
			var re *ValkeyError
			if !errors.As(e, &re) {
				t.Fatalf("key %s: want *ValkeyError, got %T %v", k, e, e)
			}
		}
	})
	...
	t.Run("clusterMGet_same_slot_two_keys_recycles_mgetcmdsp", func(t *testing.T) {
		...
		m.DoMultiFn = func(cmd ...Completed) *valkeyresults {
			...
			vals := make([]ValkeyMessage, len(args)-1)
	...
	t.Run("clusterMGet_skips_mgetcmdsp_Put_on_non_valkey_error", func(t *testing.T) {
		ctx, cancel := context.WithCancel(context.Background())
		cancel()
		m, client := cluster()
		m.DoMultiFn = func(cmd ...Completed) *valkeyresults {
	...
	t.Run("clusterJsonMGet_same_slot_two_keys_recycles_mgetcmdsp", func(t *testing.T) {
		...
		m.DoMultiFn = func(cmd ...Completed) *valkeyresults {
			...
			vals := make([]ValkeyMessage, len(args)-2)
	...
	t.Run("clusterJsonMGet_skips_mgetcmdsp_Put_on_non_valkey_error", func(t *testing.T) {
		ctx, cancel := context.WithCancel(context.Background())
		cancel()
		m, client := cluster()
		m.DoMultiFn = func(cmd ...Completed) *valkeyresults {
