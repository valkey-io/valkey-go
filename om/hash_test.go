...
func TestNewHashRepository(t *testing.T) {
	...
	t.Run("Save", func(t *testing.T) {
		...
		t.Run("Alter Index", func(t *testing.T) {
			...
			time.Sleep(time.Second)
			_, _, err = repo.Search(ctx, func(search FtSearchIndex) valkey.Completed {
...
func TestCreateAndAliasIndex(t *testing.T) {
	...
	t.Run("CreateAndAliasIndex", func(t *testing.T) {
		err := repo.CreateAndAliasIndex(ctx, func(schema FtCreateSchema) valkey.Completed {
			return schema.FieldName("Val").Text().Build()
		})
		if err != nil {
...
func verifyAliasTarget(t *testing.T, ctx context.Context, client valkey.Client, aliasName string, expectedIndex string) {
	t.Helper()
