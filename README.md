...
This can take a long time and hurt performance. Use the normal `Do()` and `DoMulti()` instead unless you want to avoid allocating memory for large valkey response.
...
If you have many valkey connections, you may find that they occupy quite amount of memory.
...
directly into Valkey without conversion. And the `valkey.BinaryString` helper can convert `[]byte` to `string` without copy. For example:
...
type of Valkey response will be returned from the server beforehand and which parser they should use. Otherwise, it panics. 
