...
func TestClusterClientMovedRetry(t *testing.T) {
	...
	t.Run("DoMulti Retry on MOVED", func(t *testing.T) {
		...
		m.DoMultiFn = func(multi ...Completed) *redisresults {
			attempts++
			if attempts == 1 {
				return &redisresults{s: []ValkeyResult{newResult(ValkeyMessage{typ: '-', string: "MOVED 0 127.0.0.1"}, nil)}}
			}
			return &redisresults{s: []ValkeyResult{newResult(ValkeyMessage{typ: '+', string: "OK"}, nil)}}
		}
	...
	t.Run("DoMulti Retry on ASK", func(t *testing.T) {
		...
		m.DoMultiFn = func(multi ...Completed) *redisresults {
			attempts++
			if attempts == 1 {
				return &redisresults{s: []ValkeyResult{newResult(ValkeyMessage{typ: '-', string: "ASK 0 127.0.0.1"}, nil)}}
			}
			return &redisresults{s: []ValkeyResult{newResult(ValkeyMessage{typ: '+', string: "OK"}, nil), newResult(ValkeyMessage{typ: '+', string: "OK"}, nil)}}
		}
