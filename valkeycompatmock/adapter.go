...
type ClientMock interface {
	...
	ExpectBitCount(key string, bitCount *valkeycompat.BitCount) *ExpectedInt
	...
	ExpectSort(key string, sort *valkeycompat.Sort) *ExpectedStringSlice
	ExpectSortInterfaces(key string, sort *valkeycompat.Sort) *ExpectedSlice
	ExpectSortRO(key string, sort *valkeycompat.Sort) *ExpectedStringSlice
	ExpectSortStore(key, store string, sort *valkeycompat.Sort) *ExpectedInt
	...
	ExpectLCS(q *valkeycompat.LCSQuery) *ExpectedLCS
	...
	ExpectLPos(key string, element string, a valkeycompat.LPosArgs) *ExpectedInt
	ExpectLPosCount(key string, element string, count int64, a valkeycompat.LPosArgs) *ExpectedIntSlice
	...
	ExpectSetArgs(key string, value any, a valkeycompat.SetArgs) *ExpectedStatus
	...
	ExpectGeoRadius(key string, longitude, latitude float64, query *valkeycompat.GeoRadiusQuery) *ExpectedGeoLocation
	ExpectGeoRadiusByMember(key, member string, query *valkeycompat.GeoRadiusQuery) *ExpectedGeoLocation
	ExpectGeoRadiusByMemberStore(key, member string, query *valkeycompat.GeoRadiusQuery) *ExpectedInt
	ExpectGeoRadiusStore(key string, longitude, latitude float64, query *valkeycompat.GeoRadiusQuery) *ExpectedInt
	ExpectGeoSearch(key string, q *valkeycompat.GeoSearchQuery) *ExpectedStringSlice
	ExpectGeoSearchLocation(key string, q *valkeycompat.GeoSearchLocationQuery) *ExpectedGeoSearchLocation
	ExpectGeoSearchStore(src, dest string, q *valkeycompat.GeoSearchStoreQuery) *ExpectedInt
	...
	ExpectZRangeByLex(key string, zRangeBy *valkeycompat.ZRangeBy) *ExpectedStringSlice
	ExpectZRangeByScore(key string, zRangeBy *valkeycompat.ZRangeBy) *ExpectedStringSlice
	ExpectZRangeByScoreWithScores(key string, zRangeBy *valkeycompat.ZRangeBy) *ExpectedZSlice
	ExpectZRangeStore(dst string, z valkeycompat.ZRangeArgs) *ExpectedInt
	ExpectZRevRangeByLex(key string, zRangeBy *valkeycompat.ZRangeBy) *ExpectedStringSlice
	ExpectZRevRangeByScore(key string, zRangeBy *valkeycompat.ZRangeBy) *ExpectedStringSlice
	ExpectZRevRangeByScoreWithScores(key string, zRangeBy *valkeycompat.ZRangeBy) *ExpectedZSlice
	...
	ExpectZAdd(key string, members ...valkeycompat.Z) *ExpectedInt
	ExpectZAddNX(key string, members ...valkeycompat.Z) *ExpectedInt
	ExpectZAddXX(key string, members ...valkeycompat.Z) *ExpectedInt
	ExpectZAddLT(key string, members ...valkeycompat.Z) *ExpectedInt
	ExpectZAddGT(key string, members ...valkeycompat.Z) *ExpectedInt
	ExpectZAddArgs(key string, args valkeycompat.ZAddArgs) *ExpectedInt
	ExpectZAddArgsIncr(key string, args valkeycompat.ZAddArgs) *ExpectedFloat
	ExpectZInter(store *valkeycompat.ZStore) *ExpectedStringSlice
	ExpectZInterWithScores(store *valkeycompat.ZStore) *ExpectedZSlice
	ExpectZInterStore(destination string, store *valkeycompat.ZStore) *ExpectedInt
	ExpectZUnion(store valkeycompat.ZStore) *ExpectedStringSlice
	ExpectZUnionWithScores(store valkeycompat.ZStore) *ExpectedZSlice
	ExpectZUnionStore(dest string, store *valkeycompat.ZStore) *ExpectedInt
	ExpectZRangeArgs(z valkeycompat.ZRangeArgs) *ExpectedStringSlice
	ExpectZRangeArgsWithScores(z valkeycompat.ZRangeArgs) *ExpectedZSlice
	...
	ExpectFunctionList(q valkeycompat.FunctionListQuery) *ExpectedFunctionList
...
func NewClientMock() (*Client, ClientMock) {
	...
	raw.EXPECT().Nodes().Return(map[string]valkey.Client{"mock": raw}).AnyTimes()
	cm := newAdapter(raw)
	return &Client{Cmdable: valkeycompat.NewAdapter(raw), raw: raw, mock: cm}, cm
}
...
func (c *Client) Do(ctx context.Context, args ...any) *valkeycompat.Cmd {
	ret := &valkeycompat.Cmd{}
	if len(args) == 0 {
...
func (m *clientMock) ExpectationsWereMet() error {
	...
	if len(root.queue) > 0 {
		...
		return fmt.Errorf("valkeycompatmock: there are remaining expectations: %v", cmds)
	}
	return nil
}
...
func (e *expectation) resolve(cmd valkey.Completed) valkey.ValkeyResult {
	if e.err != nil {
...
func (m *clientMock) pushSet(matcher gomock.Matcher, result valkey.ValkeyResult) *expectation {
	return m.pushWithOptions(matcher, result, true)
}

func (m *clientMock) pushWithOptions(matcher gomock.Matcher, defaultResult valkey.ValkeyResult, resultSet bool) *expectation {
	root := m.root()
	root.mu.Lock()
	defer root.mu.Unlock()
	e := &expectation{
...
func (m *clientMock) ExpectExpire(key string, expiration time.Duration) *ExpectedBool {
	e := m.push(match("EXPIRE", key, strconv.FormatInt(formatSec(expiration), 10)), mock.Result(mock.ValkeyBool(false)))
	return &ExpectedBool{exp: e}
}

func (m *clientMock) ExpectPing() *ExpectedStatus {
	e := m.push(match("PING"), mock.Result(mock.ValkeyString("PONG")))
	return &ExpectedStatus{exp: e}
}

func (m *clientMock) ExpectEcho(message any) *ExpectedString {
	e := m.push(match("ECHO", str(message)), mock.Result(mock.ValkeyString("")))
	return &ExpectedString{exp: e}
}

func (m *clientMock) ExpectIncr(key string) *ExpectedInt {
	e := m.push(match("INCR", key), mock.Result(mock.ValkeyInt64(0)))
	return &ExpectedInt{exp: e}
}

func (m *clientMock) ExpectIncrBy(key string, value int64) *ExpectedInt {
	e := m.push(match("INCRBY", key, strconv.FormatInt(value, 10)), mock.Result(mock.ValkeyInt64(0)))
	return &ExpectedInt{exp: e}
}

func (m *clientMock) ExpectDecr(key string) *ExpectedInt {
	e := m.push(match("DECR", key), mock.Result(mock.ValkeyInt64(0)))
	return &ExpectedInt{exp: e}
}

func (m *clientMock) ExpectDecrBy(key string, value int64) *ExpectedInt {
	e := m.push(match("DECRBY", key, strconv.FormatInt(value, 10)), mock.Result(mock.ValkeyInt64(0)))
	return &ExpectedInt{exp: e}
}

func (m *clientMock) ExpectMGet(keys ...string) *ExpectedSlice {
	args := append([]string{"MGET"}, keys...)
	e := m.push(match(args...), mock.Result(mock.ValkeyArray()))
...
func (m *clientMock) ExpectHGet(key, field string) *ExpectedString {
	e := m.push(match("HGET", key, field), mock.Result(mock.ValkeyNil()))
	return &ExpectedString{exp: e}
}
...
func (m *clientMock) ExpectHGetAll(key string) *ExpectedMapStringString {
	e := m.push(match("HGETALL", key), mock.Result(mock.ValkeyMap(map[string]valkey.ValkeyMessage{})))
...
func (m *clientMock) ExpectLPop(key string) *ExpectedString {
	e := m.push(match("LPOP", key), mock.Result(mock.ValkeyNil()))
	return &ExpectedString{exp: e}
}

func (m *clientMock) ExpectRPop(key string) *ExpectedString {
	e := m.push(match("RPOP", key), mock.Result(mock.ValkeyNil()))
	return &ExpectedString{exp: e}
}

func (m *clientMock) ExpectLLen(key string) *ExpectedInt {
	e := m.push(match("LLEN", key), mock.Result(mock.ValkeyInt64(0)))
	return &ExpectedInt{exp: e}
}
...
func (m *clientMock) ExpectSMembers(key string) *ExpectedStringSlice {
	e := m.push(match("SMEMBERS", key), mock.Result(mock.ValkeyArray()))
	return &ExpectedStringSlice{exp: e}
}

func (m *clientMock) ExpectEval(script string, keys []string, args ...any) *ExpectedCmd {
	e := m.push(evalMatcher(script, keys, args...), mock.Result(mock.ValkeyNil()))
...
func (m *clientMock) ExpectBitCount(key string, bitCount *valkeycompat.BitCount) *ExpectedInt {
	var cmd valkey.Completed
	if bitCount != nil {
...
func (m *clientMock) ExpectTime() *ExpectedTime {
	...
	e := m.push(match(cmd.Commands()...), mock.Result(mock.ValkeyArray(mock.ValkeyString("0"), mock.ValkeyString("0"))))
...
func (m *clientMock) ExpectEvalSha(sha string, keys []string, args ...any) *ExpectedCmd {
	...
	e := m.push(match(cmd.Commands()...), mock.Result(mock.ValkeyNil()))
	return &ExpectedCmd{exp: e}
}
...
func normalizeFilter(filter any) valkeycompat.FilterBy {
	switch f := filter.(type) {
	case valkeycompat.FilterBy:
		return f
	case *valkeycompat.FilterBy:
		if f != nil {
			return *f
...
func (m *clientMock) ExpectClientPause(dur time.Duration) *ExpectedBool {
	...
	e := m.push(match(cmd.Commands()...), mock.Result(mock.ValkeyString("OK")))
...
func (m *clientMock) ExpectSort(key string, sort *valkeycompat.Sort) *ExpectedStringSlice {
	cmd := sortCmd(m.raw, "SORT", key, normalizeSort(sort), "")
	e := m.push(match(cmd.Commands()...), defaultArrayResult())
	return &ExpectedStringSlice{exp: e}
}

func (m *clientMock) ExpectSortInterfaces(key string, sort *valkeycompat.Sort) *ExpectedSlice {
	cmd := sortCmd(m.raw, "SORT", key, normalizeSort(sort), "")
	e := m.push(match(cmd.Commands()...), defaultArrayResult())
	return &ExpectedSlice{exp: e}
}

func (m *clientMock) ExpectSortRO(key string, sort *valkeycompat.Sort) *ExpectedStringSlice {
	cmd := sortCmd(m.raw, "SORT_RO", key, normalizeSort(sort), "")
	e := m.push(match(cmd.Commands()...), defaultArrayResult())
	return &ExpectedStringSlice{exp: e}
}

func (m *clientMock) ExpectSortStore(key, store string, sort *valkeycompat.Sort) *ExpectedInt {
	cmd := sortCmd(m.raw, "SORT", key, normalizeSort(sort), store)
	e := m.push(match(cmd.Commands()...), defaultIntResult())
	return &ExpectedInt{exp: e}
}

func normalizeSort(sort any) valkeycompat.Sort {
	switch s := sort.(type) {
	case valkeycompat.Sort:
		return s
	case *valkeycompat.Sort:
		if s != nil {
			return *s
	...
	return valkeycompat.Sort{}
}

func sortCmd(raw valkey.Client, command, key string, sort valkeycompat.Sort, store string) valkey.Completed {
...
func (m *clientMock) ExpectLCS(q *valkeycompat.LCSQuery) *ExpectedLCS {
	var cmd valkey.Completed
	var readType uint8
	...
	var defaultResult valkey.ValkeyResult
	switch readType {
	case 2:
...
func (m *clientMock) ExpectLPos(key string, element string, a valkeycompat.LPosArgs) *ExpectedInt {
...
func (m *clientMock) ExpectLPosCount(key string, element string, count int64, a valkeycompat.LPosArgs) *ExpectedIntSlice {
...
func (m *clientMock) ExpectSetArgs(key string, value any, a valkeycompat.SetArgs) *ExpectedStatus {
...
func (m *clientMock) ExpectGeoRadius(key string, longitude, latitude float64, query *valkeycompat.GeoRadiusQuery) *ExpectedGeoLocation {
...
func (m *clientMock) ExpectGeoRadiusByMember(key, member string, query *valkeycompat.GeoRadiusQuery) *ExpectedGeoLocation {
...
func (m *clientMock) ExpectGeoRadiusByMemberStore(key, member string, query *valkeycompat.GeoRadiusQuery) *ExpectedInt {
...
func (m *clientMock) ExpectGeoRadiusStore(key string, longitude, latitude float64, query *valkeycompat.GeoRadiusQuery) *ExpectedInt {
...
func (m *clientMock) ExpectGeoSearch(key string, q *valkeycompat.GeoSearchQuery) *ExpectedStringSlice {
...
func (m *clientMock) ExpectGeoSearchLocation(key string, q *valkeycompat.GeoSearchLocationQuery) *ExpectedGeoSearchLocation {
	cmd := m.raw.B().Arbitrary("GEOSEARCH").Keys(key)
...
func (m *clientMock) ExpectGeoSearchStore(src, dest string, q *valkeycompat.GeoSearchStoreQuery) *ExpectedInt {
...
func normalizeGeoLocation(loc any) valkeycompat.GeoLocation {
	switch l := loc.(type) {
	case valkeycompat.GeoLocation:
		return l
	case *valkeycompat.GeoLocation:
		if l != nil {
			return *l
	...
	return valkeycompat.GeoLocation{}
}

func normalizeGeoRadiusQuery(q any) valkeycompat.GeoRadiusQuery {
	switch query := q.(type) {
	case valkeycompat.GeoRadiusQuery:
		return query
	case *valkeycompat.GeoRadiusQuery:
		if query != nil {
			return *query
	...
	return valkeycompat.GeoRadiusQuery{}
}

func normalizeGeoSearchQuery(q any) valkeycompat.GeoSearchQuery {
	switch query := q.(type) {
	case valkeycompat.GeoSearchQuery:
		return query
	case *valkeycompat.GeoSearchQuery:
		if query != nil {
			return *query
	...
	return valkeycompat.GeoSearchQuery{}
}

func normalizeGeoSearchLocationQuery(q any) valkeycompat.GeoSearchLocationQuery {
	switch query := q.(type) {
	case valkeycompat.GeoSearchLocationQuery:
		return query
	case *valkeycompat.GeoSearchLocationQuery:
		if query != nil {
			return *query
	...
	return valkeycompat.GeoSearchLocationQuery{}
}

func normalizeGeoSearchStoreQuery(q any) valkeycompat.GeoSearchStoreQuery {
	switch query := q.(type) {
	case valkeycompat.GeoSearchStoreQuery:
		return query
	case *valkeycompat.GeoSearchStoreQuery:
		if query != nil {
			return *query
	...
	return valkeycompat.GeoSearchStoreQuery{}
}

// mirrors valkeycompat.GeoRadiusQuery.args(); keep in sync if upstream changes.
func geoRadiusQueryArgs(q valkeycompat.GeoRadiusQuery) []string {
...
// mirrors valkeycompat.GeoSearchQuery.args(); keep in sync if upstream changes.
func geoSearchQueryArgs(q valkeycompat.GeoSearchQuery) []string {
	args := make([]string, 0, 2)
	if q.Member != "" {
...
// mirrors valkeycompat.GeoSearchLocationQuery.args(); keep in sync if upstream changes.
func geoSearchLocationQueryArgs(q valkeycompat.GeoSearchLocationQuery) []string {
...
func (m *clientMock) ExpectZRangeByLex(key string, zRangeBy *valkeycompat.ZRangeBy) *ExpectedStringSlice {
...
func (m *clientMock) ExpectZRangeByScore(key string, zRangeBy *valkeycompat.ZRangeBy) *ExpectedStringSlice {
	opt := normalizeZRangeBy(zRangeBy)
	var cmd valkey.Completed
	if opt.Offset != 0 || opt.Count != 0 {
...
func (m *clientMock) ExpectZRangeByScoreWithScores(key string, zRangeBy *valkeycompat.ZRangeBy) *ExpectedZSlice {
	opt := normalizeZRangeBy(zRangeBy)
	var cmd valkey.Completed
	if opt.Offset != 0 || opt.Count != 0 {
...
func (m *clientMock) ExpectZRangeStore(dst string, z valkeycompat.ZRangeArgs) *ExpectedInt {
...
func (m *clientMock) ExpectZRevRangeByLex(key string, zRangeBy *valkeycompat.ZRangeBy) *ExpectedStringSlice {
	opt := normalizeZRangeBy(zRangeBy)
	var cmd valkey.Completed
	if opt.Offset != 0 || opt.Count != 0 {
...
func (m *clientMock) ExpectZRevRangeByScore(key string, zRangeBy *valkeycompat.ZRangeBy) *ExpectedStringSlice {
	opt := normalizeZRangeBy(zRangeBy)
	var cmd valkey.Completed
	if opt.Offset != 0 || opt.Count != 0 {
...
func (m *clientMock) ExpectZRevRangeByScoreWithScores(key string, zRangeBy *valkeycompat.ZRangeBy) *ExpectedZSlice {
	opt := normalizeZRangeBy(zRangeBy)
	var cmd valkey.Completed
	if opt.Offset != 0 || opt.Count != 0 {
...
func (m *clientMock) ExpectZAdd(key string, members ...valkeycompat.Z) *ExpectedInt {
	cmd := compatZAddArgs(m.raw.B(), key, false, valkeycompat.ZAddArgs{Members: members})
	e := m.push(match(cmd.Commands()...), defaultIntResult())
	return &ExpectedInt{exp: e}
}

func (m *clientMock) ExpectZAddNX(key string, members ...valkeycompat.Z) *ExpectedInt {
	cmd := compatZAddArgs(m.raw.B(), key, false, valkeycompat.ZAddArgs{Members: members, NX: true})
	e := m.push(match(cmd.Commands()...), defaultIntResult())
	return &ExpectedInt{exp: e}
}

func (m *clientMock) ExpectZAddXX(key string, members ...valkeycompat.Z) *ExpectedInt {
	cmd := compatZAddArgs(m.raw.B(), key, false, valkeycompat.ZAddArgs{Members: members, XX: true})
	e := m.push(match(cmd.Commands()...), defaultIntResult())
	return &ExpectedInt{exp: e}
}

func (m *clientMock) ExpectZAddLT(key string, members ...valkeycompat.Z) *ExpectedInt {
	cmd := compatZAddArgs(m.raw.B(), key, false, valkeycompat.ZAddArgs{Members: members, LT: true})
	e := m.push(match(cmd.Commands()...), defaultIntResult())
	return &ExpectedInt{exp: e}
}

func (m *clientMock) ExpectZAddGT(key string, members ...valkeycompat.Z) *ExpectedInt {
	cmd := compatZAddArgs(m.raw.B(), key, false, valkeycompat.ZAddArgs{Members: members, GT: true})
	e := m.push(match(cmd.Commands()...), defaultIntResult())
	return &ExpectedInt{exp: e}
}

func (m *clientMock) ExpectZAddArgs(key string, args valkeycompat.ZAddArgs) *ExpectedInt {
	cmd := compatZAddArgs(m.raw.B(), key, false, args)
	e := m.push(match(cmd.Commands()...), defaultIntResult())
	return &ExpectedInt{exp: e}
}

func (m *clientMock) ExpectZAddArgsIncr(key string, args valkeycompat.ZAddArgs) *ExpectedFloat {
	cmd := compatZAddArgs(m.raw.B(), key, true, args)
	e := m.push(match(cmd.Commands()...), defaultFloatResult())
	return &ExpectedFloat{exp: e}
}

func (m *clientMock) ExpectZInter(store *valkeycompat.ZStore) *ExpectedStringSlice {
	cmd := compatZStore(m.raw.B().Arbitrary("ZINTER"), normalizeZStore(store)).ReadOnly()
	e := m.push(match(cmd.Commands()...), defaultArrayResult())
	return &ExpectedStringSlice{exp: e}
}

func (m *clientMock) ExpectZInterWithScores(store *valkeycompat.ZStore) *ExpectedZSlice {
	cmd := compatZStore(m.raw.B().Arbitrary("ZINTER"), normalizeZStore(store)).Args("WITHSCORES").ReadOnly()
	e := m.push(match(cmd.Commands()...), defaultArrayResult())
	return &ExpectedZSlice{exp: e}
}

func (m *clientMock) ExpectZInterStore(destination string, store *valkeycompat.ZStore) *ExpectedInt {
	cmd := compatZStore(m.raw.B().Arbitrary("ZINTERSTORE").Keys(destination), normalizeZStore(store)).Build()
	e := m.push(match(cmd.Commands()...), defaultIntResult())
	return &ExpectedInt{exp: e}
}

func (m *clientMock) ExpectZUnion(store valkeycompat.ZStore) *ExpectedStringSlice {
	cmd := compatZStore(m.raw.B().Arbitrary("ZUNION"), store).ReadOnly()
	e := m.push(match(cmd.Commands()...), defaultArrayResult())
	return &ExpectedStringSlice{exp: e}
}

func (m *clientMock) ExpectZUnionWithScores(store valkeycompat.ZStore) *ExpectedZSlice {
	cmd := compatZStore(m.raw.B().Arbitrary("ZUNION"), store).Args("WITHSCORES").ReadOnly()
	e := m.push(match(cmd.Commands()...), defaultArrayResult())
	return &ExpectedZSlice{exp: e}
}

func (m *clientMock) ExpectZUnionStore(dest string, store *valkeycompat.ZStore) *ExpectedInt {
	cmd := compatZStore(m.raw.B().Arbitrary("ZUNIONSTORE").Keys(dest), normalizeZStore(store)).Build()
	e := m.push(match(cmd.Commands()...), defaultIntResult())
	return &ExpectedInt{exp: e}
}

func normalizeZStore(store any) valkeycompat.ZStore {
	switch s := store.(type) {
	case valkeycompat.ZStore:
		return s
	case *valkeycompat.ZStore:
		if s != nil {
			return *s
	...
	return valkeycompat.ZStore{}
}

func normalizeZRangeBy(zRangeBy any) valkeycompat.ZRangeBy {
	switch z := zRangeBy.(type) {
	case valkeycompat.ZRangeBy:
		return z
	case *valkeycompat.ZRangeBy:
		if z != nil {
			return *z
	...
	return valkeycompat.ZRangeBy{}
}

func (m *clientMock) ExpectZRangeArgs(z valkeycompat.ZRangeArgs) *ExpectedStringSlice {
	cmd := compatZRangeArgs(m.raw.B(), false, z)
	e := m.push(match(cmd.Commands()...), defaultArrayResult())
	return &ExpectedStringSlice{exp: e}
}

func (m *clientMock) ExpectZRangeArgsWithScores(z valkeycompat.ZRangeArgs) *ExpectedZSlice {
...
func (m *clientMock) ExpectZRangeWithScores(key string, start, stop int64) *ExpectedZSlice {
	cmd := compatZRangeArgs(m.raw.B(), true, valkeycompat.ZRangeArgs{
		Key:   key,
		Start: start,
		Stop:  stop,
	})
...
// mirrors the unexported zAddArgs helper in valkeycompat; keep in sync if upstream changes.
func compatZAddArgs(b valkey.Builder, key string, incr bool, args valkeycompat.ZAddArgs) valkey.Completed {
...
// mirrors the unexported zstore helper in valkeycompat; keep in sync if upstream changes.
func compatZStore(cmd cmds.Arbitrary, store valkeycompat.ZStore) cmds.Arbitrary {
...
// mirrors the unexported zRangeArgs helper in valkeycompat; keep in sync if upstream changes.
func compatZRangeArgs(b valkey.Builder, withScores bool, z valkeycompat.ZRangeArgs) valkey.Completed {
...
func (m *clientMock) ExpectFunctionList(q valkeycompat.FunctionListQuery) *ExpectedFunctionList {
...
func normalizeXAddArgs(args any) valkeycompat.XAddArgs {
	switch a := args.(type) {
	case valkeycompat.XAddArgs:
		return a
	case *valkeycompat.XAddArgs:
		if a != nil {
			return *a
	...
	return valkeycompat.XAddArgs{}
}

func normalizeXAutoClaimArgs(args any) valkeycompat.XAutoClaimArgs {
	switch a := args.(type) {
	case valkeycompat.XAutoClaimArgs:
		return a
	case *valkeycompat.XAutoClaimArgs:
		if a != nil {
			return *a
	...
	return valkeycompat.XAutoClaimArgs{}
}

func normalizeXClaimArgs(args any) valkeycompat.XClaimArgs {
	switch a := args.(type) {
	case valkeycompat.XClaimArgs:
		return a
	case *valkeycompat.XClaimArgs:
		if a != nil {
			return *a
	...
	return valkeycompat.XClaimArgs{}
}

func normalizeXPendingExtArgs(args any) valkeycompat.XPendingExtArgs {
	switch a := args.(type) {
	case valkeycompat.XPendingExtArgs:
		return a
	case *valkeycompat.XPendingExtArgs:
		if a != nil {
			return *a
	...
	return valkeycompat.XPendingExtArgs{}
}

func normalizeXReadArgs(args any) valkeycompat.XReadArgs {
	switch a := args.(type) {
	case valkeycompat.XReadArgs:
		return a
	case *valkeycompat.XReadArgs:
		if a != nil {
			return *a
	...
	return valkeycompat.XReadArgs{}
}

func normalizeXReadGroupArgs(args any) valkeycompat.XReadGroupArgs {
	switch a := args.(type) {
	case valkeycompat.XReadGroupArgs:
		return a
	case *valkeycompat.XReadGroupArgs:
		if a != nil {
			return *a
	...
	return valkeycompat.XReadGroupArgs{}
}
...
func (m *clientMock) ExpectXReadStreams(streams ...string) *ExpectedXStreamSlice {
	return m.ExpectXRead(&valkeycompat.XReadArgs{Streams: streams, Block: -1})
}
...
// mirrors the unexported xTrim helper in valkeycompat; keep in sync if upstream changes.
func compatXTrim(b valkey.Builder, key, strategy string, approx bool, threshold string, limit int64) valkey.Completed {
...
func (m *clientMock) ExpectTxPipeline() {
	m.pushSet(match("MULTI"), mock.Result(mock.ValkeyString("OK")))
}

func (m *clientMock) ExpectTxPipelineExec() *ExpectedSlice {
	e := m.pushSet(match("EXEC"), mock.Result(mock.ValkeyArray()))
	return &ExpectedSlice{exp: e}
}
...
// primes ROLE=master for valkeycompat fanout helpers (doPrimaries et al).
func (m *clientMock) expectRoleMaster() {
	cmd := m.raw.B().Role().Build()
	m.pushSet(match(cmd.Commands()...), mock.Result(mock.ValkeyArray(mock.ValkeyString("master"))))
}
