...
func TestNewAdapter(t *testing.T) {
	...
	}
	{
		...
		if err := rdb.Get(ctx, "missing").Err(); !errors.Is(err, valkeycompat.Nil) {
	...
	{
		cm.ExpectSet("k", "v", valkeycompat.KeepTTL).SetVal("OK")
		if err := rdb.Set(ctx, "k", "v", valkeycompat.KeepTTL).Err(); err != nil {
			t.Fatalf("unexpected err %v", err)
		}
	}
	{
	...
	{
		cm.ExpectSetNX("k", "v", valkeycompat.KeepTTL).SetVal(true)
		if ok, err := rdb.SetNX(ctx, "k", "v", valkeycompat.KeepTTL).Result(); err != nil || !ok {
			t.Fatalf("unexpected val %v err %v", ok, err)
		}
	}
	{
	...
	}
	{
		...
		if err := rdb.Eval(ctx, "return nil", []string{}).Err(); !errors.Is(err, valkeycompat.Nil) {
			t.Fatalf("unexpected err %v", err)
		}
	}
	{
...
func TestPipeline(t *testing.T) {
	...
	}
	{
		...
		if err := c3.Err(); !errors.Is(err, valkeycompat.Nil) {
			t.Fatalf("unexpected err %v", err)
		}
	}
	{
...
func TestRedismockRuntimeParity(t *testing.T) {
	...
	if err := rdb.Get(ctx, "nil").Err(); !errors.Is(err, valkeycompat.Nil) {
		t.Fatalf("unexpected err %v", err)
	}
	...
	cm.ExpectSlowLogGet(1).SetVal([]valkeycompat.SlowLog{{
		ID:         1,
		Time:       time.Unix(1, 0),
	...
	cm.ExpectFunctionList(valkeycompat.FunctionListQuery{}).SetVal([]valkeycompat.Library{{
		Name:      "lib",
		Engine:    "LUA",
		Functions: []valkeycompat.Function{{Name: "fn"}},
	}})
	libs, err := rdb.FunctionList(ctx, valkeycompat.FunctionListQuery{}).Result()
	if err != nil || len(libs) != 1 || len(libs[0].Functions) != 1 || libs[0].Functions[0].Flags == nil {
		t.Fatalf("unexpected libraries %v err %v", libs, err)
	}

	geoQuery := valkeycompat.GeoSearchLocationQuery{
		GeoSearchQuery: valkeycompat.GeoSearchQuery{Member: "origin", Radius: 10, RadiusUnit: "km"},
		WithCoord:      true,
		WithDist:       true,
		WithHash:       true,
	}
	cm.ExpectGeoSearchLocation("geo", &geoQuery).SetVal([]valkeycompat.GeoLocation{{
		Name:      "place",
		Longitude: 1.25,
		Latitude:  2.5,
		Dist:      3.75,
	...
	cm.ExpectXReadStreams("s1", "s2", "0", "0").SetVal([]valkeycompat.XStream{
		{Stream: "s2", Messages: []valkeycompat.XMessage{{ID: "2-0", Values: map[string]any{"b": "2", "a": "1"}}}},
		{Stream: "s1", Messages: []valkeycompat.XMessage{{ID: "1-0", Values: map[string]any{"a": "1"}}}},
	})
...
func TestStringCommands(t *testing.T) {
	...
	cases := []func() error{
		...
		func() error {
			cm.ExpectBitCount("k", &valkeycompat.BitCount{Start: 0, End: 10}).SetVal(5)
...
func TestServerAdminCommands(t *testing.T) {
	...
	cases := []func() error{
		...
		func() error {
			cm.ExpectCommand().SetVal([]*valkeycompat.CommandInfo{{Name: "get"}})
		...
		func() error {
			cm.ExpectCommandGetKeysAndFlags("SET", "k", "v").SetVal([]valkeycompat.KeyFlags{{Key: "k", Flags: []string{"RW"}}})
			_, err := rdb.CommandGetKeysAndFlags(ctx, "SET", "k", "v").Result()
			return err
		},
		func() error {
			cm.ExpectCommandList(&valkeycompat.FilterBy{Module: "mod"}).SetVal([]string{"cmd"})
			_, err := rdb.CommandList(ctx, valkeycompat.FilterBy{Module: "mod"}).Result()
			return err
		},
	}
	runCases(t, cm, cases)
...
func TestClientClusterCommands(t *testing.T) {
	...
	cases := []func() error{
		...
		func() error {
			cm.ExpectClusterLinks().SetVal([]valkeycompat.ClusterLink{{
				Direction:           "to",
				Node:                "abc",
				CreateTime:          1,
				Events:              "r",
		...
		func() error {
			cm.ExpectClusterShards().SetVal([]valkeycompat.ClusterShard{{
				Slots: []valkeycompat.SlotRange{{Start: 0, End: 16383}},
				Nodes: []valkeycompat.Node{{ID: "abc", Role: "master", Port: 6379}},
			}})
		...
		func() error {
			cm.ExpectClusterSlots().SetVal([]valkeycompat.ClusterSlot{{
				Start: 0,
				End:   16383,
				Nodes: []valkeycompat.ClusterNode{{Addr: "127.0.0.1:6379", ID: "abc"}},
			}})
...
func TestGenericBitmapScanSortCommands(t *testing.T) {
	...
	sort := valkeycompat.Sort{Order: "ASC"}
...
func TestListSetHashStringCommands(t *testing.T) {
	...
	cases := []func() error{
		...
		func() error {
			cm.ExpectHRandFieldWithValues("h", 2).SetVal([]valkeycompat.KeyValue{{Key: "f1", Value: "v1"}})
			_, err := rdb.HRandFieldWithValues(ctx, "h", 2).Result()
			return err
		},
		func() error {
			q := &valkeycompat.LCSQuery{Key1: "k1", Key2: "k2"}
			cm.ExpectLCS(q).SetVal(&valkeycompat.LCSMatch{MatchString: "match"})
			_, err := rdb.LCS(ctx, q).Result()
			return err
		},
		func() error {
			q := &valkeycompat.LCSQuery{Key1: "k1", Key2: "k2", Len: true}
			cm.ExpectLCS(q).SetVal(&valkeycompat.LCSMatch{Len: 5})
			_, err := rdb.LCS(ctx, q).Result()
			return err
		},
		func() error {
			q := &valkeycompat.LCSQuery{Key1: "k1", Key2: "k2", Idx: true, MinMatchLen: 4, WithMatchLen: true}
			cm.ExpectLCS(q).SetVal(&valkeycompat.LCSMatch{Len: 6, Matches: []valkeycompat.LCSMatchedPosition{{Key1: valkeycompat.LCSPosition{Start: 0, End: 3}, Key2: valkeycompat.LCSPosition{Start: 1, End: 4}, MatchLen: 4}}})
			_, err := rdb.LCS(ctx, q).Result()
			return err
		},
		func() error {
		...
		func() error {
			cm.ExpectLPos("l", "v", valkeycompat.LPosArgs{Rank: 1}).SetVal(0)
			_, err := rdb.LPos(ctx, "l", "v", valkeycompat.LPosArgs{Rank: 1}).Result()
			return err
		},
		func() error {
			cm.ExpectLPosCount("l", "v", 2, valkeycompat.LPosArgs{Rank: 1}).SetVal([]int64{0, 3})
			_, err := rdb.LPosCount(ctx, "l", "v", 2, valkeycompat.LPosArgs{Rank: 1}).Result()
			return err
		},
		func() error {
		...
		func() error {
			cm.ExpectSetArgs("k", "v", valkeycompat.SetArgs{TTL: time.Second, Mode: "NX"}).SetVal("OK")
			return rdb.SetArgs(ctx, "k", "v", valkeycompat.SetArgs{TTL: time.Second, Mode: "NX"}).Err()
		},
	}
	runCases(t, cm, cases)
}
...
func TestGeoCommands(t *testing.T) {
	...
	loc := valkeycompat.GeoLocation{Name: "k1", Longitude: 122.4194, Latitude: 37.7749}
	radiusQuery := valkeycompat.GeoRadiusQuery{Radius: 200}
	radiusStoreQuery := valkeycompat.GeoRadiusQuery{Radius: 200, StoreDist: "result"}
	searchQuery := valkeycompat.GeoSearchQuery{Member: "Catania", BoxWidth: 400, BoxHeight: 100, BoxUnit: "km", Sort: "asc"}
	searchLocationQuery := valkeycompat.GeoSearchLocationQuery{
		GeoSearchQuery: valkeycompat.GeoSearchQuery{Longitude: 15, Latitude: 37, Radius: 200, RadiusUnit: "km", Sort: "asc"},
		WithCoord:      true,
		WithDist:       true,
		WithHash:       true,
	}
	searchStoreQuery := valkeycompat.GeoSearchStoreQuery{
		GeoSearchQuery: valkeycompat.GeoSearchQuery{Longitude: 15, Latitude: 37, Radius: 200, RadiusUnit: "km", Sort: "asc"},
		StoreDist:      false,
	}
	...
	cases := []func() error{
		...
		func() error {
			cm.ExpectGeoPos("1", "1", "2").SetVal([]*valkeycompat.GeoPos{{Longitude: 1, Latitude: 2}})
			_, err := rdb.GeoPos(ctx, "1", "1", "2").Result()
			return err
		},
		func() error {
			cm.ExpectGeoRadius("1", 1, 2, &radiusQuery).SetVal([]valkeycompat.GeoLocation{{Name: "m1"}})
			_, err := rdb.GeoRadius(ctx, "1", 1, 2, radiusQuery).Result()
			return err
		},
		func() error {
			cm.ExpectGeoRadiusByMember("1", "2", &radiusQuery).SetVal([]valkeycompat.GeoLocation{{Name: "m1"}})
		...
		func() error {
			cm.ExpectGeoSearchLocation("1", &searchLocationQuery).SetVal([]valkeycompat.GeoLocation{{Name: "m1"}})
...
func TestSortedSetCommandsExtra(t *testing.T) {
	...
	opt := valkeycompat.ZRangeBy{Min: "0", Max: "10"}
	optLimit := valkeycompat.ZRangeBy{Min: "0", Max: "10", Offset: 1, Count: 2}
	rangeArgs := valkeycompat.ZRangeArgs{Key: "z", Start: int64(0), Stop: int64(-1)}

	cases := []func() error{
		func() error {
			cm.ExpectBZMPop(time.Second, "MIN", 1, "z").SetVal("z", []valkeycompat.Z{{Member: "m", Score: 1}})
			_, _, err := rdb.BZMPop(ctx, time.Second, "MIN", 1, "z").Result()
			return err
		},
		func() error {
			cm.ExpectBZPopMax(time.Second, "z").SetVal(&valkeycompat.ZWithKey{Key: "z", Z: valkeycompat.Z{Member: "m", Score: 1}})
			_, err := rdb.BZPopMax(ctx, time.Second, "z").Result()
			return err
		},
		func() error {
			cm.ExpectBZPopMin(time.Second, "z").SetVal(&valkeycompat.ZWithKey{Key: "z", Z: valkeycompat.Z{Member: "m", Score: 1}})
		...
		func() error {
			cm.ExpectZDiffWithScores("k1", "k2").SetVal([]valkeycompat.Z{{Member: "m", Score: 1}})
		...
		func() error {
			cm.ExpectZMPop("MIN", 1, "z").SetVal("z", []valkeycompat.Z{{Member: "m", Score: 1}})
		...
		func() error {
			cm.ExpectZRandMemberWithScores("z", 2).SetVal([]valkeycompat.Z{{Member: "m", Score: 1}})
		...
		func() error {
			cm.ExpectZRangeByScoreWithScores("z", &opt).SetVal([]valkeycompat.Z{{Member: "m", Score: 1}})
			_, err := rdb.ZRangeByScoreWithScores(ctx, "z", opt).Result()
			return err
		},
		func() error {
			cm.ExpectZRangeByScoreWithScores("z", &optLimit).SetVal([]valkeycompat.Z{{Member: "m", Score: 1}})
		...
		func() error {
			cm.ExpectZRevRangeByScoreWithScores("z", &opt).SetVal([]valkeycompat.Z{{Member: "m", Score: 1}})
			_, err := rdb.ZRevRangeByScoreWithScores(ctx, "z", opt).Result()
			return err
		},
		func() error {
			cm.ExpectZRevRangeByScoreWithScores("z", &optLimit).SetVal([]valkeycompat.Z{{Member: "m", Score: 1}})
			_, err := rdb.ZRevRangeByScoreWithScores(ctx, "z", optLimit).Result()
			return err
		},
		func() error {
			cm.ExpectZRevRangeWithScores("z", 0, -1).SetVal([]valkeycompat.Z{{Member: "m", Score: 1}})
...
func TestSortedSetHelpersCommands(t *testing.T) {
	...
	member := valkeycompat.Z{Member: "m", Score: 1}
	store := valkeycompat.ZStore{Keys: []string{"k1", "k2"}, Weights: []int64{1, 2}, Aggregate: "SUM"}
	rangeArgs := valkeycompat.ZRangeArgs{Key: "z", Start: int64(0), Stop: int64(-1)}
	...
	cases := []func() error{
		...
		func() error {
			cm.ExpectZAddArgs("z", valkeycompat.ZAddArgs{Members: []valkeycompat.Z{member}, Ch: true}).SetVal(1)
			_, err := rdb.ZAddArgs(ctx, "z", valkeycompat.ZAddArgs{Members: []valkeycompat.Z{member}, Ch: true}).Result()
			return err
		},
		func() error {
			cm.ExpectZAddArgsIncr("z", valkeycompat.ZAddArgs{Members: []valkeycompat.Z{member}}).SetVal(2.0)
			_, err := rdb.ZAddArgsIncr(ctx, "z", valkeycompat.ZAddArgs{Members: []valkeycompat.Z{member}}).Result()
			return err
		},
		func() error {
		...
		func() error {
			cm.ExpectZInterWithScores(&store).SetVal([]valkeycompat.Z{{Member: "m", Score: 1}})
		...
		func() error {
			cm.ExpectZUnionWithScores(store).SetVal([]valkeycompat.Z{{Member: "m", Score: 1}})
		...
		func() error {
			cm.ExpectZRangeArgsWithScores(rangeArgs).SetVal([]valkeycompat.Z{{Member: "m", Score: 1}})
			_, err := rdb.ZRangeArgsWithScores(ctx, rangeArgs).Result()
			return err
		},
		func() error {
			cm.ExpectZRangeWithScores("z", 0, -1).SetVal([]valkeycompat.Z{{Member: "m", Score: 1}})
...
func TestPubSubScriptingFunctionsCommands(t *testing.T) {
	...
	cases := []func() error{
		...
		func() error {
			cm.ExpectFunctionList(valkeycompat.FunctionListQuery{
				LibraryNamePattern: "*",
				WithCode:           true,
			}).SetVal([]valkeycompat.Library{{
				Name:   "lib",
				Engine: "LUA",
				Code:   "code",
				Functions: []valkeycompat.Function{{
					Name:        "fn",
					Description: "desc",
					Flags:       []string{"no-writes"},
				}},
			}})
			_, err := rdb.FunctionList(ctx, valkeycompat.FunctionListQuery{
				LibraryNamePattern: "*",
				WithCode:           true,
			}).Result()
			return err
...
func TestStreamsCommands(t *testing.T) {
	...
	xAddArgs := valkeycompat.XAddArgs{
		Stream: "s",
		Values: map[string]string{"f": "v"},
	}
	xAutoClaimArgs := valkeycompat.XAutoClaimArgs{
		Stream:   "s",
		Group:    "g",
		Consumer: "c",
		Start:    "0-0",
	...
	xClaimArgs := valkeycompat.XClaimArgs{
		Stream:   "s",
		Group:    "g",
		Consumer: "c",
		Messages: []string{"1-0"},
	...
	xPendingExtArgs := valkeycompat.XPendingExtArgs{
		Stream: "s",
		Group:  "g",
		Start:  "-",
		End:    "+",
	...
	xReadArgs := valkeycompat.XReadArgs{
		Streams: []string{"s", "0"},
		Count:   10,
		Block:   -1,
	}
	...
	xMsg := valkeycompat.XMessage{ID: "1-0", Values: map[string]any{"f": "v"}}
	xStream := valkeycompat.XStream{Stream: "s", Messages: []valkeycompat.XMessage{xMsg}}
	...
	cases := []func() error{
		...
		func() error {
			cm.ExpectXAutoClaim(&xAutoClaimArgs).SetVal([]valkeycompat.XMessage{xMsg}, "0-0")
		...
		func() error {
			cm.ExpectXClaim(&xClaimArgs).SetVal([]valkeycompat.XMessage{xMsg})
		...
		func() error {
			cm.ExpectXInfoConsumers("s", "g").SetVal([]valkeycompat.XInfoConsumer{{Name: "c", Pending: 1}})
			_, err := rdb.XInfoConsumers(ctx, "s", "g").Result()
			return err
		},
		func() error {
			cm.ExpectXInfoGroups("s").SetVal([]valkeycompat.XInfoGroup{{Name: "g"}})
			_, err := rdb.XInfoGroups(ctx, "s").Result()
			return err
		},
		func() error {
			cm.ExpectXInfoStream("s").SetVal(&valkeycompat.XInfoStream{Length: 1})
			_, err := rdb.XInfoStream(ctx, "s").Result()
			return err
		},
		func() error {
			cm.ExpectXInfoStreamFull("s", 10).SetVal(&valkeycompat.XInfoStreamFull{Length: 1})
		...
		func() error {
			...
			cm.ExpectXInfoStreamFull("s", 10).SetVal(&valkeycompat.XInfoStreamFull{
				Length: 1,
				Groups: []valkeycompat.XInfoStreamGroup{{
					Name:            "g",
					LastDeliveredID: "1-0",
					EntriesRead:     1,
					Lag:             2,
					PelCount:        1,
					Pending: []valkeycompat.XInfoStreamGroupPending{{
						ID:            "1-0",
						Consumer:      "c",
						DeliveryTime:  deliveryTime,
						DeliveryCount: 1,
					}},
					Consumers: []valkeycompat.XInfoStreamConsumer{{
						Name:     "c",
						SeenTime: seenTime,
						PelCount: 1,
						Pending: []valkeycompat.XInfoStreamConsumerPending{{
							ID:            "1-0",
							DeliveryTime:  deliveryTime,
							DeliveryCount: 1,
						}},
		...
		func() error {
			cm.ExpectXPending("s", "g").SetVal(&valkeycompat.XPending{Count: 1, Lower: "0-0", Higher: "1-0"})
			_, err := rdb.XPending(ctx, "s", "g").Result()
			return err
		},
		func() error {
			cm.ExpectXPendingExt(&xPendingExtArgs).SetVal([]valkeycompat.XPendingExt{{ID: "1-0", Consumer: "c"}})
			_, err := rdb.XPendingExt(ctx, xPendingExtArgs).Result()
			return err
		},
		func() error {
			cm.ExpectXRange("s", "-", "+").SetVal([]valkeycompat.XMessage{xMsg})
			_, err := rdb.XRange(ctx, "s", "-", "+").Result()
			return err
		},
		func() error {
			cm.ExpectXRangeN("s", "-", "+", 10).SetVal([]valkeycompat.XMessage{xMsg})
			_, err := rdb.XRangeN(ctx, "s", "-", "+", 10).Result()
			return err
		},
		func() error {
			cm.ExpectXRead(&xReadArgs).SetVal([]valkeycompat.XStream{xStream})
			_, err := rdb.XRead(ctx, xReadArgs).Result()
			return err
		},
		func() error {
			cm.ExpectXReadGroup(&xReadGroupArgs).SetVal([]valkeycompat.XStream{xStream})
			_, err := rdb.XReadGroup(ctx, xReadGroupArgs).Result()
			return err
		},
		func() error {
			cm.ExpectXReadStreams("s", "0").SetVal([]valkeycompat.XStream{xStream})
			_, err := rdb.XReadStreams(ctx, "s", "0").Result()
			return err
		},
		func() error {
			cm.ExpectXRevRange("s", "+", "-").SetVal([]valkeycompat.XMessage{xMsg})
			_, err := rdb.XRevRange(ctx, "s", "+", "-").Result()
			return err
		},
		func() error {
			cm.ExpectXRevRangeN("s", "+", "-", 10).SetVal([]valkeycompat.XMessage{xMsg})
...
func TestNewExpectedSetErr(t *testing.T) {
	...
	lcsQuery := &valkeycompat.LCSQuery{Key1: "k1", Key2: "k2"}
	...
	cases := []func() error{
		...
		func() error {
			cm.ExpectFunctionList(valkeycompat.FunctionListQuery{LibraryNamePattern: "*"}).SetErr(errAny)
			return wantAnyErr(rdb.FunctionList(ctx, valkeycompat.FunctionListQuery{LibraryNamePattern: "*"}).Err())
		},
		func() error {
		...
		func() error {
			cm.ExpectGeoRadius("1", 1, 2, &valkeycompat.GeoRadiusQuery{Radius: 200}).SetErr(errAny)
			return wantAnyErr(rdb.GeoRadius(ctx, "1", 1, 2, valkeycompat.GeoRadiusQuery{Radius: 200}).Err())
		},
		func() error {
...
func TestNewExpectedRedisNil(t *testing.T) {
	...
	cases := []func() error{
		func() error {
			...
			if err := rdb.BZPopMax(ctx, time.Second, "z").Err(); !errors.Is(err, valkeycompat.Nil) {
...
func TestCacheCompatRedisNil(t *testing.T) {
	...
	if err := rdb.Cache(time.Second).Get(ctx, "key").Err(); !errors.Is(err, valkeycompat.Nil) {
